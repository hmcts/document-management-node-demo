...
module "app" {
  ...
  app_settings = {
    ...
    DM_STORE_APP_URI= "http://${var.dm_store_app_url}-${local.local_env}.service.${local.ase_name}.internal"
    EM_ANNO_APP_URI="http://${var.em_anno_app_url}-${local.local_env}.service.${local.ase_name}.internal"
    EM_REDACT_APP_URI="http://${var.em_redact_app_url}-${local.local_env}.service.${local.ase_name}.internal"
